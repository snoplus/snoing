...
class Package( threading.Thread ):
    ...
    def SetDependencyPaths( self, paths ):
        """ Set the dependency path dictionay."""
    ...
    def Run( self ):
        ...
        if self._Mode == 0: # Requires downloading
            if self.Download():
                self._IncrementMode()
        elif self._Mode == 1: # Requires installing
            if self.Install():
                self._IncrementMode()
        else:
    ...
    def _IncrementMode( self ):
        ...
        self._Lock.acquire( True ) # Acquire a lock, block to wait
        self._Mode += 1
        self._Lock.release()
        return
